...
pub struct Downstream {
    ...
    monitor_token: std::cell::RefCell<Option<String>>,
}
...
impl Downstream {
    ...
    pub async fn new_downstream(
        ...
        let upstream_extranonce1 = extranonce1;
        let pending = crate::pending_extranonce::take(&downstream_addr);
        let (downstream_extranonce1, pending_user_agent) = match pending {
            Some(pending) => (pending.extranonce1, pending.user_agent),
            None => (upstream_extranonce1.clone(), String::new()),
        };

        let mut first_job = last_notify.expect("we have an assertion at the beginning of this function");
        if downstream_extranonce1.is_empty() && !upstream_extranonce1.is_empty() {
            let mut coin_base1 = first_job.coin_base1.as_ref().to_vec();
            coin_base1.extend_from_slice(upstream_extranonce1.as_ref());
            first_job.coin_base1 = coin_base1.into();
        }

        let downstream = Arc::new(Mutex::new(Downstream {
            connection_id,
            authorized_names: vec![],
            extranonce1: downstream_extranonce1,
            upstream_extranonce1,
            ...
            user_agent: std::cell::RefCell::new(pending_user_agent),
            monitor_token: std::cell::RefCell::new(Configuration::token()),
        }));

        let host = downstream_addr.to_string();
    ...
    pub async fn accept_connections(
        ...
        upstream_difficulty_config: Arc<Mutex<UpstreamDifficultyConfig>>,
        downstreams: Receiver<(Sender<String>, Receiver<String>, SocketAddr)>,
        stats_sender: StatsSender,
        ...
        match response {
            Ok(res) => {
                if let Some(r) = res {
                    ...
                    Self::send_message_downstream(self_.clone(), r.into()).await;
                    Ok(())
                } else {
    ...
    pub(super) fn patch_notify_for_downstream(&self, notify: &mut Notify<'static>) {
        if self.extranonce1.is_empty() && !self.upstream_extranonce1.is_empty() {
            let mut coin_base1 = notify.coin_base1.as_ref().to_vec();
            coin_base1.extend_from_slice(self.upstream_extranonce1.as_ref());
            notify.coin_base1 = coin_base1.into();
        }
    }

    pub(super) fn monitor_token(&self) -> Option<String> {
        self.monitor_token.borrow().clone()
    }
    ...
    pub fn new(
        ...
        Downstream {
            ...
            authorized_names,
            extranonce1,
            upstream_extranonce1: extranonce1.clone(),
...
impl IsServer<'static> for Downstream {
    ...
    fn handle_authorize(&self, request: &client_to_server::Authorize) -> bool {
        let monitor_token = if !request.password.trim().is_empty() {
            request.password.clone()
        } else {
            request.name.clone()
        };
        if !monitor_token.trim().is_empty() {
            self.share_monitor.set_token(Some(monitor_token.clone()));
            self.monitor_token.replace(Some(monitor_token));
        }

        if self.authorized_names.is_empty() {
            let user_agent = self.user_agent.borrow().clone();
            let token_to_use = Configuration::token()
                .or_else(|| self.monitor_token.borrow().clone());
                ...
                if let Some(latest_difficulty) = self.difficulty_mgmt.current_difficulties.back() {
                    if met_difficulty == *latest_difficulty {
                        let to_send = SubmitShareWithChannelId {
                            ...
                            share: request.clone(),
                            extranonce: self.upstream_extranonce1.clone(),
